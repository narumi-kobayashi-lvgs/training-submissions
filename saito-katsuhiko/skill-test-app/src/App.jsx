...
const COURSES = [
  ...
  // ============================================================
  {
    ...
    concepts: [
      // 基礎
      { id: "c4-1", level: "基礎", category: "モデル・アドレス", name: "OSI/TCP-IPモデル", desc: "7層/4層モデルの各層の役割、代表プロトコルの対応" },
      { id: "c4-2", level: "基礎", category: "モデル・アドレス", name: "IPアドレスとサブネット", desc: "IPv4/IPv6の構造、CIDR表記、プライベート/パブリックアドレスの区別" },
      { id: "c4-3", level: "基礎", category: "プロトコル", name: "TCP vs UDP", desc: "コネクション型と非コネクション型の違い、信頼性・順序保証・用途の差異" },
      { id: "c4-4", level: "基礎", category: "プロトコル", name: "DNS・HTTPSの仕組み", desc: "名前解決のフロー、TLSハンドシェイクの概要" },
      { id: "c4-5", level: "基礎", category: "ネットワーク機器", name: "ルーター・スイッチ・ファイアウォールの役割", desc: "L2/L3の違い、パケットフィルタリングの基本" },
      // 応用
      { id: "c4-6", level: "応用", category: "アドレス・ルーティング", name: "サブネット設計とルーティング", desc: "サブネット分割計算、デフォルトゲートウェイ、静的/動的ルーティングの概念" },
      { id: "c4-7", level: "応用", category: "プロトコル応用", name: "HTTP/1.1・HTTP/2・HTTP/3の違い", desc: "Keep-Alive、多重化、QUICの特徴とパフォーマンスへの影響" },
      { id: "c4-8", level: "応用", category: "セキュリティ", name: "NATとポートフォワーディング", desc: "NAT変換の仕組み、NAPT、プライベートネットワークからの通信フロー" },
      { id: "c4-9", level: "応用", category: "セキュリティ", name: "VPNとゼロトラスト", desc: "IPsec/SSL-VPNの違い、ゼロトラストネットワークの考え方" },
      { id: "c4-10", level: "応用", category: "負荷分散・高可用性", name: "ロードバランサーとCDN", desc: "L4/L7ロードバランサーの違い、CDNのキャッシュ戦略とエッジ配信" },
    ],
    questions: [
      // ── 選択式（基礎5問）──
      {
        no: 1, type: "選択", level: "基礎", conceptId: "c4-1",
        q: "OSIモデルの第4層（トランスポート層）に対応するプロトコルはどれか。",
        choices: [
          "A. IP",
          "B. Ethernet",
          "C. TCP / UDP",
        ...
        answer: "C",
        point: "L4トランスポート層はTCP/UDPが担う。IP(L3)、Ethernet(L2)、HTTP(L7)はそれぞれ異なる層。",
      },
      {
        no: 2, type: "選択", level: "基礎", conceptId: "c4-2",
        q: "CIDR表記 192.168.1.0/24 のサブネットマスクとして正しいものはどれか。",
        choices: [
          "A. 255.0.0.0",
          "B. 255.255.0.0",
          "C. 255.255.255.0",
        ...
        answer: "C",
        point: "/24 はネットワーク部が24ビット。上位24ビットが1のマスクは 255.255.255.0。ホスト数は 256-2=254台。",
      },
      {
        no: 3, type: "選択", level: "基礎", conceptId: "c4-3",
        q: "UDPを使用するプロトコル・用途として適切なものはどれか。",
        choices: [
          "A. HTTPSによるWebページ取得",
          "B. SSHによるリモートログイン",
          "C. ビデオ会議・DNS・オンラインゲーム",
          "D. FTPによるファイル転送",
        ],
        answer: "C",
        point: "UDPは順序保証・再送なし。遅延が許容されず多少のパケットロスより速度が優先されるリアルタイム通信やDNS（軽量な問い合わせ）に適する。",
      },
      {
        no: 4, type: "選択", level: "基礎", conceptId: "c4-4",
        q: "DNSの名前解決において、キャッシュDNSサーバーが最初に問い合わせる先はどれか。",
        choices: [
          "A. コンテンツDNSサーバー（権威サーバー）",
          "B. ルートDNSサーバー",
          "C. TLDサーバー（.jp / .com 等）",
          "D. ISPのリゾルバ",
        ],
        answer: "B",
        point: "キャッシュがない場合、フルリゾルバはルートDNS→TLD→権威サーバーの順に反復問い合わせを行う。",
      },
      {
        no: 5, type: "選択", level: "基礎", conceptId: "c4-5",
        q: "ファイアウォールの「ステートフルインスペクション」の説明として正しいものはどれか。",
        choices: [
          "A. すべてのパケットをポート番号のみで判定する",
          "B. 通信のセッション状態を追跡し、確立済みセッションの戻りパケットを自動的に許可する",
          "C. L7アプリケーション層の内容まで解析してフィルタリングする",
          "D. 送信元IPアドレスのみを基準に許可/拒否を判定する",
        ],
        answer: "B",
        point: "ステートフルインスペクションはコネクションテーブルでセッション状態を管理し、応答パケットを動的に許可する。単純なパケットフィルタとは異なる。",
      },
      // ── 選択式（応用5問）──
      {
        no: 6, type: "選択", level: "応用", conceptId: "c4-7",
        q: "HTTP/2 が HTTP/1.1 と比較して改善した点として正しいものはどれか。",
        choices: [
          "A. 暗号化が不要になった",
          "B. 1つのTCP接続で複数リクエストを並列処理（多重化）できるようになった",
          "C. UDPベースのプロトコルに切り替わった",
          "D. DNSの名前解決が不要になった",
        ],
        answer: "B",
        point: "HTTP/1.1はHOLブロッキングがあり同時リクエスト数に制限。HTTP/2はストリーム多重化で1接続上に複数リクエストを並列処理できる。HTTP/3はQUIC(UDP)ベース。",
      },
      {
        no: 7, type: "選択", level: "応用", conceptId: "c4-8",
        q: "NAPT（IPマスカレード）の説明として正しいものはどれか。",
        choices: [
          "A. 複数のパブリックIPを複数のプライベートIPに1対1で変換する",
          "B. 1つのパブリックIPとポート番号の組み合わせで複数のプライベートIPを識別し変換する",
          "C. プライベートIPを変換せずそのままルーティングする",
          "D. IPv6アドレスをIPv4アドレスに変換する",
        ],
        answer: "B",
        point: "NAPTはIPアドレス＋ポート番号のペアで変換テーブルを管理する。一般家庭やオフィスで広く使われ、1つのグローバルIPで多数の端末がインターネットを利用できる。",
      },
      {
        no: 8, type: "選択", level: "応用", conceptId: "c4-6",
        q: "192.168.10.0/25 のネットワークで使用可能なホストアドレス数はどれか。",
        choices: [
          "A. 32",
          "B. 62",
          "C. 126",
        ...
        answer: "C",
        point: "/25 はホスト部7ビット。2^7=128 からネットワークアドレスとブロードキャストの2つを引いて 126台。",
      },
      {
        no: 9, type: "選択", level: "応用", conceptId: "c4-10",
        q: "L7ロードバランサーがL4ロードバランサーと比較して実現できることはどれか。",
        choices: [
          "A. TCPレベルでの高速な負荷分散",
          "B. HTTPヘッダーやURLパスを参照したコンテンツベースのルーティング",
          "C. UDPプロトコルのみの分散処理",
          "D. IPアドレスを使わない負荷分散",
        ],
        answer: "B",
        point: "L7はHTTPのホスト名・パス・ヘッダー等を解析できるためパスベースルーティングやA/Bテスト振り分けが可能。L4はIP/ポートベースで高速だがコンテンツを見られない。",
      },
      {
        no: 10, type: "選択", level: "応用", conceptId: "c4-9",
        q: "ゼロトラストネットワークの基本的な考え方として正しいものはどれか。",
        choices: [
          "A. 社内ネットワーク内にいる端末はすべて信頼できるとして認証を省略する",
          "B. VPNさえ使えばすべての通信を信頼してよい",
          "C. ネットワークの場所に関わらず、すべてのアクセスを検証・認証してから許可する",
          "D. ファイアウォールを廃止してすべての通信を許可する",
        ],
        answer: "C",
        point: "ゼロトラストは「内部だから安全」という前提を捨て、すべてのリソースアクセスを継続的に検証する。テレワーク普及とクラウド化に伴い重要性が増している。",
      },
      // ── 記述式（基礎5問）──
      {
        no: 11, type: "記述", level: "基礎", conceptId: "c4-1",
        q: "TCP/IPの4層モデルの各層の名称と、それぞれの層が担う役割・代表的なプロトコルを説明してください。",
        answer: "①ネットワークインターフェース層：物理的な通信媒体上のフレーム送受信（Ethernet、Wi-Fi）。②インターネット層：IPアドレスを用いたパケットのルーティング（IP、ICMP）。③トランスポート層：エンドツーエンドの通信制御（TCP・信頼性あり、UDP・軽量）。④アプリケーション層：ユーザーが直接利用するプロトコル（HTTP、DNS、SMTP等）。",
      },
      {
        no: 12, type: "記述", level: "基礎", conceptId: "c4-3",
        q: "TCPの3ウェイハンドシェイクの手順を説明し、なぜこの手順が必要なのかを述べてください。",
        answer: "手順：①クライアントがSYNを送信。②サーバーがSYN-ACKを返す。③クライアントがACKを送信し接続確立。必要な理由：双方向の通信路が確立できることを確認し、シーケンス番号の初期値を同期させるため。これによりパケットの順序管理・再送・重複排除が可能になる。",
      },
      {
        no: 13, type: "記述", level: "基礎", conceptId: "c4-4",
        q: "ブラウザに https://example.com を入力してからページが表示されるまでの通信フローを、DNSからTLSハンドシェイクまで順を追って説明してください。",
        answer: "①ブラウザがキャッシュを確認し、なければOSのリゾルバがDNSサーバーに問い合わせてIPアドレスを取得。②TCPの3ウェイハンドシェイクでサーバーとの接続を確立。③TLSハンドシェイク（サーバー証明書の検証・共通鍵の交換）で暗号化セッションを確立。④HTTPリクエストを送信。⑤サーバーがHTTPレスポンスを返す。⑥ブラウザがHTMLを解析・レンダリングしてページを表示。",
      },
      {
        no: 14, type: "記述", level: "基礎", conceptId: "c4-2",
        q: "プライベートIPアドレスの範囲（RFC1918）を3つ挙げ、それぞれが主にどのような場面で使われるか説明してください。",
        answer: "①10.0.0.0/8（10.x.x.x）：大規模な企業ネットワークやクラウドVPCで使われる。アドレス空間が広い。②172.16.0.0/12（172.16〜172.31.x.x）：中規模ネットワーク。Dockerのデフォルトなどでも使われる。③192.168.0.0/16（192.168.x.x）：家庭用ルーターやオフィスの小規模LAN。最もよく目にする範囲。",
      },
      {
        no: 15, type: "記述", level: "基礎", conceptId: "c4-5",
        q: "ルーターとスイッチの違いを、動作するOSI層とその役割の観点から説明してください。",
        answer: "スイッチ（L2）：MACアドレスを使って同一ネットワーク（セグメント）内のフレームを転送する。LANスイッチが代表例。ルーター（L3）：IPアドレスを使って異なるネットワーク間でパケットをルーティングする。デフォルトゲートウェイとして機能し、インターネットとの接続やVLAN間の通信を担う。",
      },
      // ── 記述式（応用5問）──
      {
        no: 16, type: "記述", level: "応用", conceptId: "c4-6",
        q: "ある拠点に 10.0.0.0/24 のアドレスが割り当てられており、これを4つの等しいサブネットに分割したい。各サブネットのCIDR表記と使用可能なホスト数を求めてください。",
        answer: "4分割には2ビット追加が必要。/24+2=/26。各サブネット：10.0.0.0/26、10.0.0.64/26、10.0.0.128/26、10.0.0.192/26。ホスト部は6ビット（2^6=64）からネットワーク・ブロードキャストを引いて各62台。",
      },
      {
        no: 17, type: "記述", level: "応用", conceptId: "c4-7",
        q: "HTTP/1.1のHead-of-Line（HOL）ブロッキング問題を説明し、HTTP/2とHTTP/3がそれぞれどのように解決しているか述べてください。",
        answer: "HOLブロッキング：HTTP/1.1では1つのTCP接続で前のリクエストが完了するまで次が待機される。HTTP/2での解決：ストリーム多重化により1つのTCP接続上で複数リクエストを並列処理。ただしTCPレベルのHOLブロッキング（パケットロス時の全ストリーム停止）は残る。HTTP/3での解決：QUIC（UDP上に独自の信頼性制御）を採用し、TCPレベルのHOLブロッキングも解消。ストリームごとに独立して再送処理を行う。",
      },
      {
        no: 18, type: "記述", level: "応用", conceptId: "c4-10",
        q: "CDN（Content Delivery Network）の仕組みと、Webアプリのパフォーマンス・可用性向上にどのように貢献するかを説明してください。",
        answer: "CDNはエッジサーバーを世界各地に配置し、ユーザーに最も近いエッジからコンテンツを配信する。貢献①：オリジンサーバーまでのラウンドトリップが減り応答速度が向上する。貢献②：静的コンテンツ（画像・CSS・JS）をエッジでキャッシュしてオリジンの負荷を下げる。貢献③：エッジが分散しているためオリジンが落ちてもキャッシュを返せる場合があり可用性が向上する。貢献④：DDoS攻撃をエッジで吸収できる。",
      },
      {
        no: 19, type: "記述", level: "応用", conceptId: "c4-8",
        q: "NATを使っているネットワーク内のサーバーに外部から接続させるためのポートフォワーディングの仕組みを説明し、設定例を示してください。",
        answer: "ポートフォワーディング：NATルーターが特定のグローバルIPのポート番号宛ての通信を、指定したプライベートIPのポートに転送するルール。例：グローバルIP 203.0.113.1:80 への通信 → プライベート 192.168.1.10:80（Webサーバー）に転送。同様に :22 → SSH、:443 → HTTPS など複数サービスを1つのグローバルIPで公開できる。ただし不要なポートを開放するとセキュリティリスクになるため最小限に絞るべき。",
      },
      {
        no: 20, type: "記述", level: "応用", conceptId: "c4-9",
        q: "SSL-VPN と IPsec VPN の違いを説明し、リモートワーク環境でどちらが一般的に選ばれるか理由とともに述べてください。",
        answer: "IPsec VPN：ネットワーク層（L3）で暗号化。サイト間VPNに適し、専用クライアントが必要なことが多い。設定が複雑でファイアウォール通過に課題が生じることがある。SSL-VPN：TLS（L4〜L7）を使用。ブラウザやWebベースクライアントで利用でき、443番ポートを使うため多くのファイアウォールを通過しやすい。リモートワークでは：SSL-VPNが一般的。追加のポート開放不要でクライアント導入が容易なため。ゼロトラスト移行後はVPN自体を廃止しSASEやIDaaSを採用するケースも増えている。",
      },
    ],
  },
  ...
  // ============================================================
  {
    ...
    concepts: [
      // 基礎（CLF相当）
      { id: "c5-1", level: "基礎", category: "AWSの基礎概念", name: "リージョン・AZ・エッジロケーション", desc: "地理的分散の単位、可用性ゾーンによる冗長化の目的" },
      { id: "c5-2", level: "基礎", category: "コンピューティング", name: "EC2の基礎", desc: "インスタンスタイプ・AMI・セキュリティグループ・キーペアの役割" },
      { id: "c5-3", level: "基礎", category: "ネットワーク", name: "VPCの基礎", desc: "パブリック/プライベートサブネット、インターネットゲートウェイ、ルートテーブル" },
      { id: "c5-4", level: "基礎", category: "ストレージ・DB", name: "S3・RDS・DynamoDBの特徴", desc: "オブジェクトストレージ/RDB/NoSQLの使い分けと基本的なユースケース" },
      { id: "c5-5", level: "基礎", category: "セキュリティ", name: "IAMの基礎", desc: "ユーザー・グループ・ロール・ポリシーの構造と最小権限の原則" },
      // 応用（SAA相当）
      { id: "c5-6", level: "応用", category: "高可用性設計", name: "マルチAZ・Auto Scaling・ELB", desc: "ALB/NLBの使い分け、Auto Scalingのスケールイン/アウト条件、マルチAZ構成の意味" },
      { id: "c5-7", level: "応用", category: "ネットワーク応用", name: "VPCの高度な設定", desc: "NATゲートウェイ・VPCピアリング・セキュリティグループ vs NACLの違い" },
      { id: "c5-8", level: "応用", category: "コスト・パフォーマンス", name: "CloudFront・ElastiCache・コスト最適化", desc: "CDN配信とオリジン設定、キャッシュ戦略、リザーブドvsスポットインスタンス" },
      { id: "c5-9", level: "応用", category: "サーバーレス・コンテナ", name: "Lambda・ECS・ECR", desc: "サーバーレスのユースケースと制約、コンテナオーケストレーションの基本" },
      { id: "c5-10", level: "応用", category: "運用・監視", name: "CloudWatch・CloudTrail・構成管理", desc: "メトリクス/ログ/アラームの設計、証跡による操作ログの取得、IaCの概念" },
    ],
    questions: [
      // ── 選択式（基礎5問）──
      {
        no: 1, type: "選択", level: "基礎", conceptId: "c5-1",
        q: "AWSのアベイラビリティゾーン（AZ）に関して正しい説明はどれか。",
        choices: [
          "A. AZは世界に1つだけ存在し、全リージョンで共有される",
          "B. 1つのリージョン内に複数の独立したデータセンター群（AZ）が存在し、物理的に分離されている",
          "C. AZとリージョンは同義であり区別はない",
          "D. AZをまたいだデプロイはAWSでは非推奨とされている",
        ],
        answer: "B",
        point: "各AZは独立した電源・ネットワーク・物理施設を持ち、1つのAZが障害になっても他のAZで継続できる。マルチAZ構成が高可用性の基本。",
      },
      {
        no: 2, type: "選択", level: "基礎", conceptId: "c5-2",
        q: "EC2のセキュリティグループについて正しい説明はどれか。",
        choices: [
          "A. サブネット単位に適用するステートレスなファイアウォール",
          "B. インスタンス単位に適用するステートフルなファイアウォール",
          "C. VPC全体に適用されるL7ファイアウォール",
          "D. 送信ルールは設定できず受信ルールのみ管理できる",
        ],
        answer: "B",
        point: "セキュリティグループはインスタンス（ENI）に紐付くステートフルなFW。インバウンドを許可した通信の応答は自動的に許可される。サブネット単位のステートレスFWはNACL。",
      },
      {
        no: 3, type: "選択", level: "基礎", conceptId: "c5-3",
        q: "VPCのパブリックサブネットとプライベートサブネットの主な違いはどれか。",
        choices: [
          "A. パブリックサブネットはIPv4のみ、プライベートはIPv6のみ使用できる",
          "B. パブリックサブネットはインターネットゲートウェイへのルートを持ち、インターネットと直接通信できる",
          "C. プライベートサブネットはAWS外のサービスに一切アクセスできない",
          "D. パブリックサブネットにはRDSを配置できない",
        ],
        answer: "B",
        point: "パブリックサブネットのルートテーブルにインターネットゲートウェイ（IGW）へのルート（0.0.0.0/0→IGW）が設定されている点が本質的な違い。",
      },
      {
        no: 4, type: "選択", level: "基礎", conceptId: "c5-4",
        q: "Amazon S3の説明として正しいものはどれか。",
        choices: [
          "A. EC2インスタンスに直接マウントするブロックストレージ",
          "B. キーとバリューのみを扱うインメモリキャッシュサービス",
          "C. バケット内にオブジェクト（ファイル）を保存するHTTPベースのオブジェクトストレージ",
          "D. リレーショナルDBとしてSQL文を実行できるサービス",
        ],
        answer: "C",
        point: "S3はオブジェクトストレージ。ブロックストレージはEBS、RDBはRDS、インメモリキャッシュはElastiCache。S3は高耐久性（99.999999999%）で静的ファイル・バックアップ・データレイクに使われる。",
      },
      {
        no: 5, type: "選択", level: "基礎", conceptId: "c5-5",
        q: "IAMにおける「最小権限の原則」の説明として正しいものはどれか。",
        choices: [
          "A. すべてのユーザーにAdministratorAccessポリシーを付与して管理を簡略化する",
          "B. ユーザーやロールには業務上必要な最低限の権限のみを付与する",
          "C. rootアカウントを日常業務で使用し、他のユーザーには権限を与えない",
          "D. すべての権限をDenyに設定し、必要な時だけ変更する",
        ],
        answer: "B",
        point: "最小権限はセキュリティの基本原則。IAMでは必要なアクション・リソースを絞ったポリシーを作成し、ロールを通じてEC2やLambdaに付与する。rootは日常使用禁止。",
      },
      // ── 選択式（応用5問）──
      {
        no: 6, type: "選択", level: "応用", conceptId: "c5-7",
        q: "プライベートサブネット内のEC2がインターネット上のパッケージリポジトリにアクセスするために必要なAWSリソースはどれか。",
        choices: [
          "A. インターネットゲートウェイ（IGW）をプライベートサブネットのルートに追加する",
          "B. NATゲートウェイをパブリックサブネットに配置し、プライベートサブネットのデフォルトルートをNATゲートウェイに向ける",
          "C. VPCピアリングを設定する",
          "D. セキュリティグループのアウトバウンドルールをすべて許可する",
        ],
        answer: "B",
        point: "プライベートサブネットからの外部通信はNATゲートウェイ（またはNATインスタンス）経由。IGWを直接使うと外部からも到達可能になりプライベートの意味がなくなる。",
      },
      {
        no: 7, type: "選択", level: "応用", conceptId: "c5-6",
        q: "ALB（Application Load Balancer）とNLB（Network Load Balancer）の使い分けとして正しいものはどれか。",
        choices: [
          "A. ALBはUDPのみ、NLBはTCPのみに対応している",
          "B. ALBはHTTP/HTTPSのコンテンツベースルーティングに、NLBは超低レイテンシが必要なTCP/UDP通信に適している",
          "C. NLBはHTTPSを終端できるがALBはできない",
          "D. ALBはEC2にのみ、NLBはLambdaにのみ使用できる",
        ],
        answer: "B",
        point: "ALBはL7でホスト名・パスベースのルーティングやWebSocket対応。NLBはL4で数百万リクエスト/秒・超低レイテンシが必要なゲームや金融系サービスに向く。",
      },
      {
        no: 8, type: "選択", level: "応用", conceptId: "c5-9",
        q: "AWS Lambdaの制約として正しいものはどれか。",
        choices: [
          "A. 実行時間は最大15分まで",
          "B. 常時起動しているため起動時間のオーバーヘッドはない",
          "C. 使用できるプログラミング言語はPythonのみ",
          "D. VPC内には配置できない",
        ],
        answer: "A",
        point: "Lambdaの最大実行時間は15分（900秒）。コールドスタート問題あり。Node.js/Python/Go/Java等複数言語対応。VPC Lambdaも可能。長時間処理はECSやEC2が適切。",
      },
      {
        no: 9, type: "選択", level: "応用", conceptId: "c5-7",
        q: "セキュリティグループ（SG）とネットワークACL（NACL）の違いとして正しいものはどれか。",
        choices: [
          "A. SGはサブネット単位でステートレス、NACLはインスタンス単位でステートフル",
          "B. SGはインスタンス単位でステートフル、NACLはサブネット単位でステートレス",
          "C. SGとNACLは同一の機能でどちらかのみ使えばよい",
          "D. NACLは許可ルールのみ設定でき、拒否ルールは設定できない",
        ],
        answer: "B",
        point: "SGはステートフル（戻りパケット自動許可）・インスタンス単位。NACLはステートレス（戻りパケットも明示的に許可が必要）・サブネット単位。両者を組み合わせて多層防御を構成する。",
      },
      {
        no: 10, type: "選択", level: "応用", conceptId: "c5-10",
        q: "AWS CloudTrailの主な目的として正しいものはどれか。",
        choices: [
          "A. EC2のCPU使用率やメモリ使用量を監視しアラームを発行する",
          "B. AWSアカウント内のAPI呼び出し（誰が・いつ・何をしたか）を記録して監査ログを提供する",
          "C. アプリケーションのログをリアルタイムに収集・分析する",
          "D. ネットワークのパケットをキャプチャして通信内容を解析する",
        ],
        answer: "B",
        point: "CloudTrailはAPI操作の証跡を記録するセキュリティ・監査ツール。CloudWatchはメトリクス/ログ/アラームの監視。VPC Flow LogsはネットワークトラフィックのIPレベルの記録。",
      },
      // ── 記述式（基礎5問）──
      {
        no: 11, type: "記述", level: "基礎", conceptId: "c5-3",
        q: "WebアプリをAWS上に構築する際の典型的な3層VPC構成（パブリック・アプリ・DBの各サブネット）を説明し、それぞれのサブネットに配置するリソースとその理由を述べてください。",
        answer: "パブリックサブネット：ALBやNATゲートウェイを配置。インターネットから直接到達できる必要があるリソースのみ置く。アプリ（プライベート）サブネット：EC2やECSタスクを配置。外部から直接到達させずALBを経由させることでセキュリティを高める。DB（プライベート）サブネット：RDSやElastiCacheを配置。インターネットから完全に遮断し、アプリサブネットからのみアクセスを許可する。マルチAZに同じ構成を複製することで冗長性も確保する。",
      },
      {
        no: 12, type: "記述", level: "基礎", conceptId: "c5-5",
        q: "EC2インスタンスがS3バケットにアクセスするための適切な認証方法を説明し、アクセスキーをEC2上に直接配置してはいけない理由を述べてください。",
        answer: "適切な方法：IAMロールを作成しS3へのアクセスポリシーをアタッチして、EC2インスタンスプロファイルに設定する。EC2内のアプリはAWS SDKが自動的にインスタンスメタデータからIAMロールの一時クレデンシャルを取得する。直接配置がいけない理由：アクセスキーは静的な長期クレデンシャルであり、コード・Gitリポジトリ・AMIへの混入リスクがある。漏洩した場合に有効期限がなく即時悪用される。IAMロールによる一時クレデンシャルは自動ローテーションされリスクが低い。",
      },
      {
        no: 13, type: "記述", level: "基礎", conceptId: "c5-4",
        q: "Amazon RDSとEC2上に自己構築したDBの違いを、運用管理の観点から比較して説明してください。",
        answer: "RDS：OSパッチ・DBエンジンアップグレード・バックアップ・マルチAZフェイルオーバーをAWSがマネージドで提供。DBAの運用負荷が大幅に軽減される。マルチAZ設定でスタンバイへの自動フェイルオーバーも可能。自己構築DB（EC2）：OS・DBの設定・パッチ・バックアップをすべて自己管理する必要がある。特殊なDB設定や独自プラグインが必要な場合や、RDSが対応していないDBエンジンを使う場合に選択する。通常はRDSを使う方がコスト・信頼性ともに有利なケースが多い。",
      },
      {
        no: 14, type: "記述", level: "基礎", conceptId: "c5-1",
        q: "AWSの責任共有モデルを説明し、EC2を使用する場合にユーザー側が責任を持つセキュリティ領域を3つ挙げてください。",
        answer: "責任共有モデル：AWSは物理インフラ・ハイパーバイザー・グローバルネットワーク等「クラウドのセキュリティ」に責任を持つ。ユーザーは「クラウド内のセキュリティ」に責任を持つ。EC2でユーザーが責任を持つ領域：①OSのパッチ適用とアップデート管理。②セキュリティグループやNACLの設定（不要なポートを開放しない）。③EC2上にインストールしたアプリケーションのセキュリティ設定・脆弱性対応。他にIAMポリシーの設定・データの暗号化なども含む。",
      },
      {
        no: 15, type: "記述", level: "基礎", conceptId: "c5-2",
        q: "EC2のインスタンスタイプの命名規則（例：m5.xlarge）を解説し、ワークロードに応じた選択指針を説明してください。",
        answer: "命名規則：「m」はインスタンスファミリー（m=汎用、c=コンピューティング最適化、r=メモリ最適化、p=GPU等）、「5」はジェネレーション番号、「xlarge」はサイズ（nano/micro/small/medium/large/xlarge/2xlarge…）。選択指針：Webサーバー・アプリサーバー等バランスが求められる用途はmやtファミリー。CPUバウンドな処理（動画変換・科学計算）はcファミリー。大量データのインメモリ処理やDB用途はrファミリー。機械学習・GPU計算はpやgファミリー。",
      },
      // ── 記述式（応用5問）──
      {
        no: 16, type: "記述", level: "応用", conceptId: "c5-6",
        q: "Webサービスを高可用性構成にするためのAWS設計を、ALB・Auto Scaling・マルチAZ・RDSマルチAZを使って説明してください。",
        answer: "フロント：ALBを複数AZに配置し、ヘルスチェックで正常なインスタンスにのみトラフィックを分散。コンピューティング：Auto Scalingグループで最低2AZにEC2を配置。CloudWatchメトリクス（CPU等）に基づいてスケールアウト/インを自動化。DB：RDSマルチAZ設定でプライマリとスタンバイをAZ分けて配置。プライマリ障害時に自動フェイルオーバー（通常1〜2分）。全体：各層を複数AZに分散することでAZ単位の障害に耐える設計。SLAが99.99%を超えるサービスではさらにマルチリージョン構成も検討する。",
      },
      {
        no: 17, type: "記述", level: "応用", conceptId: "c5-8",
        q: "CloudFrontをALBのオリジンとして設定することで得られるメリットを3つ挙げ、キャッシュ設定の注意点も述べてください。",
        answer: "メリット①：エッジロケーションからの配信でレイテンシが低下し、アジア・欧米のユーザーにも高速に届く。②：静的コンテンツのキャッシュでオリジン（ALB/EC2）の負荷を削減できる。③：AWS ShieldによるDDoS緩和とWAFの統合が容易。注意点：動的コンテンツ（APIレスポンス・認証が必要なページ）をキャッシュすると、ユーザー間でレスポンスが混在するリスクがある。Cache-ControlヘッダーやCloudFrontのキャッシュポリシーで動的エンドポイントはキャッシュ無効（no-store）に設定する。",
      },
      {
        no: 18, type: "記述", level: "応用", conceptId: "c5-9",
        q: "コンテナアプリをAWS上で運用する方法としてECS（Fargate）とEC2上での自己管理の違いを説明し、Fargateが適したユースケースを述べてください。",
        answer: "EC2自己管理：コンテナホストのOSパッチ・dockerデーモン・スケーリング設定を自分で行う必要がある。細かいカスタマイズが可能でコスト制御もしやすい。ECS Fargate：サーバーレスコンテナ。ホストOS管理が不要でタスク定義にCPU/メモリを指定するだけで実行環境をAWSが管理。Fargateが適するユースケース：バッチ処理・バーストトラフィックへの対応・小〜中規模のWebサービス。インフラ管理コストを最小化してアプリ開発に集中したい場合。大規模かつ定常的なワークロードでコストを最適化したい場合はEC2起動タイプが有利なこともある。",
      },
      {
        no: 19, type: "記述", level: "応用", conceptId: "c5-10",
        q: "AWSでIaC（Infrastructure as Code）を実践する意義を説明し、CloudFormationとTerraformの違いを述べてください。",
        answer: "IaCの意義：インフラをコードで定義することで①Gitで変更履歴を管理できる②同一構成を何度でも再現可能（冪等性）③レビュープロセスを通じてミスを減らせる④手動作業の属人化を防ぐ。CloudFormation：AWS公式のIaCサービス。YAML/JSONでAWSリソースを定義。AWSサービスとの統合が深くサポートも厚いが、AWSリソースのみ管理対象。Terraform：HashiCorpのOSSツール。AWS以外のクラウドやオンプレも管理できるマルチクラウド対応。HCL言語を使用しモジュール再利用がしやすい。既存リソースのimportも可能。マルチクラウド環境や既存インフラへの段階的適用にはTerraformが選ばれることが多い。",
      },
      {
        no: 20, type: "記述", level: "応用", conceptId: "c5-8",
        q: "AWSのコスト最適化戦略として、リザーブドインスタンス・Savings Plans・スポットインスタンスの違いと、それぞれ適したワークロードを説明してください。",
        answer: "リザーブドインスタンス（RI）：1年または3年の利用を前払いコミットすることでオンデマンドより最大72%割引。定常的・予測可能なワークロード（本番WebサーバーやRDS）に適する。Savings Plans：特定インスタンスタイプに縛られず、時間あたりのコンピューティング費用をコミットする柔軟な割引プラン。EC2・Fargate・Lambdaをまたいで適用できる。スポットインスタンス：AWS余剰キャパシティを最大90%割引で利用できる。ただし中断の可能性があるため、中断耐性のあるバッチ処理・CI/CDパイプライン・ステートレスなワーカーに適する。本番サービスの定常部分はRIまたはSavings Plans、バースト分やバッチはスポットというハイブリッド構成がコスト効率に優れる。",
      },
    ],
  },
];
